...
def build_combos() -> list[Combo]:
    ...
    combos = [
        ...
        Combo(
            ...
            "C",
            "BX 先轉強，3 根內再出綠飄帶",
            "對照：深紅→淺紅之後才等 THT（SPEC 的反向）",
            lambda df: confirm_within(df["dr_to_lr"], df["bull_on"], n=3),
            a_exit,
        ),
        Combo(
            "C_PRE5",
            ...
            "BX 先轉強，5 根內再出綠飄帶",
            "對照：深紅→淺紅後 5 根內綠飄帶",
            lambda df: confirm_within(df["dr_to_lr"], df["bull_on"], n=5),
            a_exit,
        ),
        Combo(
            "C_BXPOS",
